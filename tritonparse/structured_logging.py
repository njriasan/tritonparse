...
class TensorBlobManager:
    ...
    def __init__(
        ...
        self.storage_quota = storage_quota if storage_quota is not None else TRITONPARSE_TENSOR_STORAGE_QUOTA
        
        # Resource statistics (tracks current run only)
        self.total_compressed_bytes = 0  # Total compressed size written in this run
        self.total_uncompressed_bytes = 0  # Total uncompressed size (for compression ratio)
        ...
        try:
            ...
            if hash_hex in self.hash_to_path_cache:
                ...
                if blob_path.exists():
                    ...
                    compression = "gzip" if str(blob_path).endswith(".bin.gz") else "none"
            ...
            if uncompressed_size >= self.compression_threshold:
                # Compress the data
                data_to_write = gzip.compress(blob_data, compresslevel=TRITONPARSE_COMPRESSION_LEVEL)
            ...
            log.debug(f"Saved tensor blob: {hash_hex} -> {blob_path} ({disk_size} bytes, compression={compression})")
    ...
    if TRITONPARSE_SAVE_TENSOR_BLOBS and root_dir:
        if TENSOR_BLOB_MANAGER is None:
            TENSOR_BLOB_MANAGER = TensorBlobManager(
                root_dir=root_dir,
                storage_quota=TRITONPARSE_TENSOR_STORAGE_QUOTA
            )
...
def init(
    ...
    # Store quota in temporary variable to pass to TensorBlobManager
    # Don't modify global variable to avoid affecting other code
    effective_quota = tensor_storage_quota if tensor_storage_quota is not None else TRITONPARSE_TENSOR_STORAGE_QUOTA
    
    # Set the quota in global var only for TensorBlobManager creation in init_logs()
