    ...
    def test_tensor_blob_manager(self):
        ...
        with tritonparse.context_manager.TritonParseManager(
            ...
            test_cases = [
                ...
                ((500*1024,), "Large 2MB"),        # 2MB > 1MB -> .bin.gz
                ((1024*1024,), "Very large 4MB"),  # 4MB > 1MB -> .bin.gz
            ]
            
            # Create tensors and run kernels
            for size, desc in test_cases:
                x = torch.zeros(size, device=self.cuda_device, dtype=torch.float32)
                y = run_kernel(x)
                y.sum()
            torch.cuda.synchronize()
            ...
            assert len(bin_files) == 2, f"Expected 2 .bin files (2KB, 400KB), got {len(bin_files)}"
            assert len(gz_files) == 2, f"Expected 2 .bin.gz files (2MB, 4MB), got {len(gz_files)}"
            
            # Verify large files are well compressed (zeros compress very well)
            for gz_file in gz_files:
                compressed_size = os.path.getsize(gz_file)
                # For all-zeros large tensors, compressed size should be much smaller than 1MB
                assert compressed_size < 500*1024, \
                    f"Compressed zeros should be <500KB, got {compressed_size} bytes"
            
            print(f"✓ Mixed sizes: {len(bin_files)} uncompressed (.bin), {len(gz_files)} compressed (.bin.gz)")
        ...
        with tritonparse.context_manager.TritonParseManager(
            ...
            # Count blob files - should only have 1 despite 3 launches
            blob_count = count_all_blobs(manager.dir_path)
            print(f"✓ Deduplication working: {blob_count} unique blob(s) for 3 launches")
        
        # === Test 3: Quota limit ===
        print("\n=== Test 3: Quota Limit ===")
        temp_output_dir_3 = tempfile.mkdtemp()
        
        # Set a very small quota (1KB)
        small_quota = 1024
        
        with tritonparse.context_manager.TritonParseManager(
            enable_trace_launch=True,
            enable_tensor_blob_storage=True,
            tensor_storage_quota=small_quota,
            out=temp_output_dir_3,
        ) as manager:
            # Create a large tensor that will exceed quota
            large_x = torch.randn((10000,), device=self.cuda_device, dtype=torch.float32)
            y1 = run_kernel(large_x)
            y1.sum()
            
            # Try to save another large tensor (should be blocked by quota)
            large_x2 = torch.randn((10000,), device=self.cuda_device, dtype=torch.float32)
        ...
        with tritonparse.context_manager.TritonParseManager(
            ...
            assert total_blobs == 0, f"Expected no blobs when storage disabled, found {total_blobs}"
        ...
        try:
            if TEST_KEEP_OUTPUT:
                ...
                for temp_dir in [temp_output_dir_1, temp_output_dir_2, temp_output_dir_3, 
                                  temp_output_dir_4]:
